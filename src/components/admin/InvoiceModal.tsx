...
export default function InvoiceModal({ order, onClose }: InvoiceModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto print:p-0 print:bg-white">
      <div className="bg-white rounded-2xl max-w-xl w-full p-6 shadow-2xl border border-stone-200 text-stone-800 print:shadow-none print:border-none print:max-w-none print:w-full">
        ...
        <div className="flex items-center justify-between border-b pb-4 mb-4 print:hidden">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-lg text-stone-900">ইনভয়েস / পার্সেল স্লিপ</h3>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handlePrint}
              className="flex items-center gap-1.5 px-3.5 py-1.5 bg-rose-600 text-white rounded-lg text-xs font-bold shadow hover:bg-rose-700 transition"
            >
        ...
        {/* Printable Area */}
        <div className="border border-stone-300 rounded-xl p-6 bg-white print:border-2 print:p-6 text-sm">
          ...
          <div className="flex justify-between items-start border-b border-stone-200 pb-4 mb-4">
            ...
            <div className="text-right">
              <div className="text-base font-bold text-stone-900">ইনভয়েস: {order.id}</div>
              ...
              <div className="mt-1 inline-block px-2 py-0.5 rounded bg-stone-100 text-stone-700 text-[11px] font-semibold border border-stone-200">
                অন-পেইজ: {order.landingPageTitle}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 bg-stone-50 p-3.5 rounded-lg border border-stone-200 mb-4 text-xs">
            <div>
              ...
              <p className="font-bold text-rose-700 text-sm mt-0.5">মোবাইল: {order.customerPhone}</p>
              <p className="text-stone-700 mt-1 whitespace-pre-line leading-relaxed">{order.customerAddress}</p>
            </div>
            <div className="border-l border-stone-200 pl-4">
              <p className="font-bold text-stone-600 uppercase tracking-wide text-[10px] mb-1">কুরিয়ার বিবরণ:</p>
              <p className="font-semibold text-stone-900">
                {order.courier ? (
                  <span className="text-emerald-700 uppercase font-bold">{order.courier.provider} Courier</span>
                ) : (
              ...
              {order.courier?.trackingCode && (
                <p className="text-stone-700 mt-1 font-mono text-xs">
                  ট্র্যাকিং কোড: <span className="font-bold text-stone-900">{order.courier.trackingCode}</span>
                </p>
              )}
              {order.courier?.consignmentId && (
                <p className="text-stone-700 font-mono text-xs">
                  কনসাইনমেন্ট আইডি: {order.courier.consignmentId}
                </p>
              )}
              <div className="mt-2 p-2 bg-rose-50 border border-rose-200 rounded text-rose-900 font-bold text-xs">
                ক্যাশ অন ডেলিভারি (COD): {order.grandTotal}৳
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-stone-200 pt-3 flex justify-end text-xs">
            <div className="w-48 space-y-1">
              ...
              <div className="flex justify-between text-sm font-bold text-stone-900 border-t border-stone-300 pt-1">
                ...
                <span className="text-rose-700">{order.grandTotal}৳</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 pt-4 border-t border-dashed border-stone-300 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-stone-700 italic">{order.notes || 'কাস্টমারকে কল করে ডেলিভারি দিন।'}</p>
            </div>
        ...
        <div className="mt-4 flex justify-end gap-2 print:hidden">
          ...
          <button
            type="button"
            onClick={handlePrint}
            className="flex items-center gap-1.5 px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-bold shadow"
          >
            <Printer className="w-4 h-4" />
            <span>প্রিন্ট করুন</span>
          </button>
